...
abstract class Concurrent2[F[+_, +_]] extends Temporal2[F] { self =>
  ...
  @inline def race[E, EE >: E: Semigroup, A, AA >: A](fa1: F[E, A], fa2: F[EE, AA]): F[EE, AA] = {
    ...
    implicit val m: Monad[F[EE, ?]] = self.monad
  ...
  @inline def raceEither[E, EE >: E, A, B](fa1: F[E, A], fa2: F[EE, B])(
    ...
    implicit val m: Concurrent2[F] = self
      ...
      winner match {
        case Left(Errors(wes)) =>
          loser.await >>= {
            ...
            case Left(Interrupt)   => raiseError(wes.reduce(SG))
            case Left(Errors(les)) => raiseError(SG.combine(wes.reduce(SG), les.reduce(SG)))
  ...
  @inline def zipWithPar[E, EE >: E, A, B, C](fa1: F[E, A], fa2: F[EE, B])(f: (A, B) => C)(
    ...
    implicit val c: Concurrent2[F] = self
    ...
    def coordinate[E1 <: EE, E2 <: EE, AA, BB](f: (AA, BB) => C)(
      ...
      winner match {
        case Left(Errors(wes)) =>
          loser.cancel >>= {
            case Right(_) | Left(Interrupt) => raiseError(wes.reduce(SG))
            case Left(Errors(les))          => raiseError(SG.combine(wes.reduce(SG), les.reduce(SG)))
            case Left(Defects(ts))          => dieWithMany(ts)
          }
        ...
        case Left(Interrupt) =>
          loser.cancel >>= {
            ...
            case Left(Errors(les))          => raiseError(les.reduce(SG))
            case Left(Defects(ts))          => dieWithMany(ts)
          }
