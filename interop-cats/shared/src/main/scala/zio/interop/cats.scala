...
abstract class CatsEffectPlatform
  ...
  trait CatsApp extends zio.ZIOAppDefault {
    implicit override val runtime: Runtime[ZEnv] = Runtime.default
  }
...
private class CatsArrow[E] extends ArrowChoice[ZIO[*, E, *]] {
  ...
  final override def compose[A, B, C](f: ZIO[B, E, C], g: ZIO[A, E, B]): ZIO[A, E, C] = g.flatMap(gP => f.provide(gP))
