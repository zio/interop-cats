  ...
  implicit val chunkStdInstances
    ...
    new Traverse[Chunk]
      ...
      override def map2[A, B, Z](fa: Chunk[A], fb: Chunk[B])(f: (A, B) => Z): Chunk[Z] =
        ...
        else fa.flatMap(a => fb.map(f(a, _))) // already O(1) if fa is empty
      ...
      override def tailRecM[A, B](a: A)(fn: A => Chunk[Either[A, B]]): Chunk[B] =
        Chunk.fromIterable(FlatMap[Vector].tailRecM(a)(fn(_).toVector))
      ...
      // UnorderedFoldable
      override def size[A](fa: Chunk[A]): Long =
        fa.size.toLong
      ...
      override def alignWith[A, B, C](fa: Chunk[A], fb: Chunk[B])(f: Ior[A, B] => C): Chunk[C] =
        fa.zipWith(fb)((a, b) => f(Ior.both(a, b))) ++ {
          if (fa.size >= fb.size) fa.drop(fb.size).map(a => f(Ior.left(a)))
          else fb.drop(fa.size).map(b => f(Ior.right(b)))
        }
...
private class ChunkOrder[A: Order] extends Order[Chunk[A]] {
...
private class ChunkPartialOrder[A: PartialOrder] extends PartialOrder[Chunk[A]] {
...
private class ChunkHash[A: Hash] extends ChunkEq[A] with Hash[Chunk[A]] {
  override def hash(xs: Chunk[A]): Int =
    StaticMethods.orderedHash(xs)
}

/** @see [[cats.kernel.instances.VectorEq]] */
private class ChunkEq[A: Eq] extends Eq[Chunk[A]] {
