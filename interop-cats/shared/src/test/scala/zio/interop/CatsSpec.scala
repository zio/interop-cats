...
class CatsSpec extends ZioSpecBase {
  ...
  // ZIO tests
  checkAllAsync(
    "Async[RIO[Has[Clock], _]]",
    implicit tc => AsyncTests[RIO[Has[Clock], _]].async[Int, Int, Int](100.millis)
  )
  checkAllAsync(
    "Async[Task]",
    { implicit tc =>
      implicit val runtime: Runtime[Has[Clock]] = Runtime(environment, platform)
      AsyncTests[Task].async[Int, Int, Int](100.millis)
    }
  )
  checkAllAsync(
    "Temporal[RIO[Has[Clock], _]]",
    implicit tc => GenTemporalTests[RIO[Has[Clock], _], Throwable].temporal[Int, Int, Int](100.millis)
  )
  checkAllAsync(
    "Temporal[Task]",
    { implicit tc =>
