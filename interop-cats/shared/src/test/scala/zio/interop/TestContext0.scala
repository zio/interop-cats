 ...
 * }}}
 */
final class TestContext0 private () extends ExecutionContext { self =>
   ...
   * [[TestContext0.timer timer]] method.
   *
   * Example:
   *
   * $timerExample
...
object TestContext0 {
  ...
  /** Builder for [[TestContext0]] instances. */
  def apply(): TestContext0 =
    new TestContext0

  /** Used internally by [[TestContext0]], represents the internal
  ...
  final case class State(
     ...
     */
    private[TestContext0] def execute(runnable: Runnable): State = {
      val newID = lastID + 1
     ...
     */
    private[TestContext0] def scheduleOnce(
  ...
  /**
   * Used internally by [[TestContext0]], represents a unit of work
   * pending execution.
   */
   ...
   */
  private[TestContext0] object Task {
