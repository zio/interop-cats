...
import zio.{ Queue as ZQueue, Runtime, ZTraceElement }
...
object Queue {
   ...
   */
  final def unbounded[F[+_]: Async, A](implicit R: Runtime[Any], trace: ZTraceElement): F[Queue[F, A]] =
  ...
  private[interop] final def apply[F[+_]: Async, A](
    ...
    new Queue[F, A] {
      ...
      def capacity: Int                                                                  =
        underlying.capacity
      def isEmpty(implicit trace: zio.ZTraceElement): F[Boolean]                         =
        underlying.isEmpty.toEffect[F]
      def isFull(implicit trace: zio.ZTraceElement): F[Boolean]                          =
      ...
      def takeBetween(min: Int, max: Int)(implicit trace: zio.ZTraceElement): F[List[A]] =
        underlying.takeBetween(min, max).map(_.toList).toEffect[F]
      def takeN(n: Int)(implicit trace: zio.ZTraceElement): F[List[A]]                   =
