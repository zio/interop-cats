...
final class STM[F[+_], +A] private[stm] (private[stm] val underlying: ZSTM[Throwable, A]) {
   ...
   */
  def commit(implicit R: Runtime[Any], A: Async[F], trace: ZTraceElement): F[A] =
    STM.atomically(this)
...
object STM {
  ...
  final def atomically[F[+_], A](stm: => STM[F, A])(implicit R: Runtime[Any], F: Async[F], trace: ZTraceElement): F[A] =
    F.async_ { cb =>
